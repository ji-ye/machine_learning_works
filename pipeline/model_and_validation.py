...
def main(df, features_ls, label_str,
         models_ls=['RF','DT','KNN', 'ET', 'AB', 'GB', 'LR', 'NB'],
         grid_size='test'):
    """
    Args:
        df (df): the DataFrame
        features_ls (ls of strs):
    """
    clfs, grid = define_clfs_params(grid_size)
    report_df = clf_loop(models_ls, clfs, grid,
                               X=df[features_ls], y=df[label_str])
    # manually change this to match the model you run
    report_df.to_pickle('report_ET.pkl')
    return report_df


if __name__ == '__main__':
    # manually change params to choose the model to run
    main(DF,
         features_ls=FEATURES,
         label_str=LABEL_STR,
         grid_size='large',
         models_ls=['ET'])#['RF','DT','KNN', 'ET', 'LR'])
    # report_df
