...
Code adapted from Rayid Ghani's magicloop.py for building classifiers.

The main difference is that I am using LinearSVC in place of SVC. Doing so helps
...
# The original function in Rayid's code.
'''
...
def clf_loop(models_to_run, clfs, grid, X, y):
    results_df =  pd.DataFrame(columns=('model_type','clf', 'parameters','accuracy', 'time', 'auc-roc','p, r, f1_at_5', 'p, r, f1_at_10', 'p, r, f1_at_20'))
    for n in range(1, 2):
...
#             print("index=", index, 'clf type =', type(clf))
            ...
            for p in ParameterGrid(parameter_values):
                try:
                    ...
                    accuracy = clf.fit(X_test, y_test)
                    end = time()
